...
public class DataParsingBenchmarks {
    ...
    public void Setup() {
        ...
        compiledRegex =
            new Regex(pattern, RegexOptions.Singleline | RegexOptions.Compiled,
                TimeSpan.FromSeconds(10)); // DEMO M6 - Adding Timespan
    }
    ...
    public void WithRegex() {
        Results.Clear();
        foreach (var row in SampleData.Rows) {
            if (HistoricalSalesData.TryCreateFromRow(row, compiledRegex!, CultureInfo, out var result)) {
                Results.Add(result);
            }
        }
    }
